...
export async function joinFamilyByCode(inviteCode: string): Promise<{
  ...
  if (result.error)
    return { familyId: null, familyName: null, error: result.error };
...
export async function getFamilyTransactions(
  ...
  const { type, search, page = 1, pageSize = 10 } = filters;
