...
function getDateGroup(dateStr: string): string {
  ...
  const diff    = dateDay - today  // positive = future, negative = past

  const oneDayMs = 86400000

  if (diff === 0)            return "Today"
  if (diff === oneDayMs)     return "Tomorrow"
  if (diff === -oneDayMs)    return "Yesterday"

  if (diff < 0 && diff >= -6 * oneDayMs) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => { prepend(payload.new as Notification) }
      )
      .subscribe()
    ...
    <div ref={dropRef} className="relative">
      ...
      {open && (
        <>
          ...
          <div
            className={cn(
              "z-50 bg-white border border-stone-200 shadow-[0_8px_32px_rgba(0,0,0,0.12)] overflow-hidden",
              "fixed left-0 right-0 top-auto bottom-0 rounded-t-2xl sm:rounded-2xl",
              "sm:absolute sm:fixed-none sm:bottom-auto sm:left-auto sm:right-0 sm:top-10 sm:w-80",
            )}
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-stone-100">
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  className="sm:hidden w-7 h-7 rounded-lg flex items-center justify-center text-stone-400 hover:text-stone-700 hover:bg-stone-100 transition-colors ml-1"
                >
                  <X size={13} />
                </button>
              </div>
            ...
            <div className="max-h-[50vh] sm:max-h-[360px] overflow-y-auto">
              {loading ? (
                ...
                groups.map(({ label, items }) => (
                  <div key={label}>
                    <div className="sticky top-0 px-4 py-1.5 bg-stone-50/90 backdrop-blur-sm border-b border-stone-100 z-10">
                      <p className="mono text-[9px] tracking-[0.12em] uppercase text-stone-400 font-medium">{label}</p>
                    </div>
            ...
            <div className="h-safe-area-inset-bottom sm:hidden" style={{ paddingBottom: "env(safe-area-inset-bottom)" }} />
          </div>
        </>
      )}
    </div>
