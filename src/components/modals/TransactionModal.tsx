...
export default function TransactionModal({ open, onClose, onDone, transaction }: Props) {
  const { user }  = useAuth()
  ...
  useEffect(() => {
    if (open && !isEdit && accounts.length > 0 && !form.getValues("account_id")) {
      form.setValue("account_id", accounts[0].id)
    }
  }, [open, accounts, isEdit])
  ...
  const onSubmit = async (data: TransactionForm) => {
    ...
    const cleaned: TransactionForm = {
      ...data,
      category:      data.category      || undefined,
      note:          data.note          || undefined,
      to_account_id: data.type === "transfer" ? (data.to_account_id || undefined) : undefined,
    }

    if (isEdit && transaction) {
      const error = await updateTransaction(transaction.id, cleaned)
      if (error) {
      ...
      const { error } = await createTransaction(user.id, cleaned)
      if (error) {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-stone-100">
          <div>
            ...
            <p className="mono text-[10px] text-stone-400 mt-0.5">
              {isEdit
                ? `Editing · ${new Date(transaction!.date + "T00:00:00").toLocaleDateString("en-PH", { month: "long", day: "numeric", year: "numeric" })}`
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="px-6 py-5 flex flex-col gap-4">
          ...
          {watchType !== "transfer" && (
            <SettingsSelect label="Category" icon={Tag} {...form.register("category")}>
          ...
          <div className="flex gap-2 justify-end pt-1">
            ...
            >
              {form.formState.isSubmitting
                ? <SpinnerBtn label={isEdit ? "Saving" : "Adding"} />
                : isEdit ? "Save changes" : `Add ${watchType}`
              }
            </Button>
          </div>
        </form>
